  ...
  deleteJob(hotel_id){
    if(this.network.noConnection()){
       this.network.showNetworkAlert()
          ...
          let options = new RequestOptions({ headers: headers });
            this.http.delete("http://forehotels.com:3000/api/personal_assistance/"+user_id+"/"+hotel_id, options)
                  .subscribe(data =>{
                    if(this.total > 0){             
   ...
   addToCart(id){
     if(this.network.noConnection()){
        this.network.showNetworkAlert()
          ...
          let options = new RequestOptions({ headers: headers });
            this.http.get("http://forehotels.com:3000/api/personal_assistance/"+user_id, options)
                  .subscribe(data =>{
                ...
                if(applied==false){
                  this.http
                  .post('http://forehotels.com:3000/api/pa', body, options)
                  .map(res => res.json())
                  .subscribe(
                  detail => {              
                    let alert = this.alertCtrl.create({
