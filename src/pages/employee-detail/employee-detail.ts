       ...
       if(this.network.noConnection()){
        ...
        this.hash = val          
                let headers = new Headers({
                  ...
                  this.http.get("http://www.forehotels.com:3000/api/employee/"+this.emp, options)
                        .subscribe(data =>{
  ...
  downloadCV(){
     if(this.network.noConnection()){
        this.network.showNetworkAlert()
            ...
            let options = new RequestOptions({ headers: headers });
            this.http
                .post('http://www.forehotels.com:3000/api/cv', body, options)
                .subscribe(
                    data => {
                    },
                  );
        ...
        this.network.showNetworkAlert()
            ...
            if(checker == 1){
              this.toast.show(`Candidate already shortlisted`, '5000', 'bottom').subscribe(
                ...
                let options = new RequestOptions({ headers: headers });
                this.http
                    .post('http://www.forehotels.com:3000/api/emp_wishlist', body, options)
                    .map(res => res.json())
                    .subscribe(
                        data => {
                          this.color = "#f2a900"
